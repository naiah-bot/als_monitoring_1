...
<head>
    ...
    <style>
        /* Styling for tables and layout */
        table {
            width: 70%; /* Adjust width to make it narrower */
            margin-left: auto; /* Align to the right */
            margin-right: 10%;  /* Remove left margin */
        ...
        /* Floating container for absence dates */
        .absence-modal {
            display: none;
            position: fixed;
            top: 20%;
...
<body>
    ...
    <div class="container">
        ...
        <div class="main-content">
            <h1>Admin Attendance Summary</h1>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>Total Present</th>
                            <th>Total Absent</th>
                            <th></th> <!-- Add Action column for View button -->
                        </tr>
                    </thead>
                    <tbody>
    ...
    <!-- Floating modal for absence dates -->
    <div id="absenceModal" class="absence-modal">
        <div class="absence-modal-content">
            <span class="close-btn">&times;</span>
            <h3>Absence Dates</h3>
            <div id="absenceDates"></div> <!-- Display absence dates here -->
        </div>
    </div>
    ...
    <script>
        ...
        // Show modal with absence dates
        document.querySelectorAll('.view-btn').forEach(button => {
            button.addEventListener('click', function () {
                const studentId = this.getAttribute('data-student-id');
                fetchAbsenceDates(studentId);
            });
        });

        // Fetch absence dates for the clicked student
        function fetchAbsenceDates(studentId) {
            fetch('get_absence_dates.php?student_id=' + studentId)
                .then(response => response.json())
                .then(data => {
                    const absenceDatesDiv = document.getElementById('absenceDates');
                    absenceDatesDiv.innerHTML = ''; // Clear previous data

                    if (data.length > 0) {
                        data.forEach(date => {
                            const dateElement = document.createElement('p');
                            dateElement.textContent = date.date;
                            absenceDatesDiv.appendChild(dateElement);
                        });
                    } else {
                        absenceDatesDiv.textContent = 'No absences recorded.';
                    }
